...
dat1<-dat %>% 
  tidyr::pivot_longer(-c(year,Point_ID,x_coord,y_coord,elevation_m,
                         claydepth_cm, distance_rtk_m),
                      names_to="species",
                      values_to = "presence") %>%
...
p1 <- ggplot(data=dat2, aes(x=Point_ID, y=layer_thickness_cm, fill=soil_layer)) +
               geom_area() +
  coord_cartesian(ylim=c(95, 125)) #zooming into the graph without removing data
p1
