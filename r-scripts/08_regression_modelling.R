...
orchdat3 |> 
  dplyr::group_by(year) |>
  dplyr::summarise(wa_elevation=weighted.mean(elevation_m, w=CountSum, na.rm=TRUE))
...
# y=b0 + b1x1 + b2*I(x^2)    #expect that b2 is negative 
...
anova(m1,m2) # p-value < 0.05, so m2 is significantly better than m1
...
# include the interaction (elevation*factor(year)) between elevation and year (as a factor) in the model 
...
anova(m5, test = "Chisq") #significant, use Chi-square test!!!
...
#account for the ecological optimum by adding a quadratic term for elevation_m
...
# m7 is better than m6

#now add the interaction between elevation and year
...
anova(m8,m7, test = "Chisq") # m8 is better than m7
anova(m8, test = "Chisq")
...
#geom_smooth can also plot binomials and other distributions, even glm's
